...
Watch the [Windows ZIP installation video](media/One_step_install.mp4) (25 seconds) for ZIP extraction, running `install_APACE.bat`, the integration check, and opening A-PACE in the browser. The recording reuses Python and uv already installed on the computer; a first installation that downloads them can take longer.
